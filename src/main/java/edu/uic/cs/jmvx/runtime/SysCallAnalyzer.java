 ...
 */
public class SysCallAnalyzer {

    public static ObjectInputStream in;
    public static ConcurrentHashMap<StackTraceElement, List<StackTraceElement[]>> found;
     ...
     * @param maxDepth
     */
    public static void printStack(StackTraceElement[] stack, int maxDepth) {
    ...
    public static void printSpecific(String func) throws Exception {
        ...
        try {
            for (StackTraceElement[] stack : found.get(key)) {
                System.out.println("---------");
                printStack(stack, stack.length); //depth
            }
    ...
    public static void dumpAll() throws Exception {
        for (Map.Entry<StackTraceElement, List<StackTraceElement[]>> e : found.entrySet()) {
     ...
     * @throws Exception
     */
    public static ConcurrentHashMap<StackTraceElement, List<StackTraceElement[]>> filterMap(int depth) throws Exception {
        ...
        found.replaceAll((k, v) -> filterList(depth, (LinkedList<StackTraceElement[]>) v));
        return found;
    }
     ...
     * @return
     */
    public static List<StackTraceElement[]> filterList(int depth, LinkedList<StackTraceElement[]> stacks) {
        LinkedList<StackTraceElement[]> filtered = new LinkedList<>();
        StackTraceElement[] stack;
        while ((stack = stacks.pollFirst()) != null) {
            //hack is lambda safe
            final StackTraceElement[] hack = stack;
             ...
             */
            if (!stacks.stream().anyMatch(s -> similarStack(hack, s, depth))
            )//&& notProtected(stack))
                filtered.add(stack);//filterStack(stack));
        }
        return filtered;
    ...
    public static void mergeMapIntoFound(ConcurrentHashMap<StackTraceElement, List<StackTraceElement[]>> a) {
    ...
    public static StackTraceElement simpleWrapper(StackTraceElement[] stack) {
    ...
    public static void main(String args[]) throws Exception{
        ...
        for(argi++; argi < args.length; argi++){ //for(String filename: args) {
            ...
            try {
                ...
                while (true) {
                    ConcurrentHashMap<StackTraceElement, List<StackTraceElement[]>> nd =
                            (ConcurrentHashMap<StackTraceElement, List<StackTraceElement[]>>) in.readObject();
