#!/bin/bash

BMS=("avrora" "batik" "fop" "jython" "jme" "pmd" "sunflow" "xalan")
SKIP=("h2" "luindex" "lusearch")
COLS=("nosync" "rep")
RESULTS_DIR="results"

if [ "$1" != "all" ]; then
    BMS=("$1")
fi

if [ "$2" != "all" ]; then
    COLS=("$2")
fi

N=$3

if [ "$4" != "" ]; then
    RESULTS_DIR=$4
fi

THREADS=9
if [ "$5" != "" ]; then
    THREADS=$5
fi

DACAPO_ARGS="-t $THREADS -n 1 --preserve"
TIMEOUT="timeout -k 10 10m"
TIMEOUTLONG="timeout -k 10 20m"
TIME=`which time`

DELAY=30
SCRIPT_DIR="./experiments/dacapo-chopin"

mkdir $RESULTS_DIR &> /dev/null

exptemplate() {
    MODE=$1
    RECSCRIPT=$2
    REPSCRIPT=$3
    LOGPREFIX=$4
    if [[ " ${COLS[*]} " =~ "$MODE" ]]; then
        for B in ${BMS[@]}; do
            echo "Recording run $B"
            rm -rf rec &> /dev/null
            mkdir rec &> /dev/null
    	    CMD="$TASKSET_LEADER $TIMEOUT $SCRIPT_DIR/$RECSCRIPT $B $DACAPO_ARGS"
    	    echo "$CMD"
    	    bash -c "$CMD"
            ls -lah rec
            du -s rec
    
	    LOG="$RESULTS_DIR/$LOGPREFIX-$B.txt"
	    CMD="$TASKSET_LEADER $TIMEOUT $SCRIPT_DIR/$REPSCRIPT $B $DACAPO_ARGS"
	    echo "" > $LOG
            for I in $(seq $N); do
                echo "Executing replayer $B run $I"
		echo "$CMD" |& tee -a $LOG
		eval "$CMD" |& tee -a $LOG 
            done
        done
    fi
}

exptemplate "rep" "run-recorder.sh" "run-replayer.sh" "jmvx-rep"
exptemplate "nosync" "run-recorder-nosync.sh" "run-replayer-nosync.sh" "jmvx-rep-nosync"

